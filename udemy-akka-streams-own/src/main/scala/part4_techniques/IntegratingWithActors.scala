...
Actors as a flow
Flow[Int].ask[String](parallelism = 5)(myActorRef)
mySource.ask[Int](parallelism = 5)(myActorRef) <-- equivalent with "via an ask-based flow"

Actors as a source
Source.actorRef[Int](bufferSize = 16, overflowStrategy = OverflowStrategy.dropHead) <-- materializes to an ActorRef

Actors as a sink
Sink.actorRefWithAck[Int](
  myActorRef,
  onInitMessage = MyStreamInit,
  onCompleteMessage = MyStreamComplete,
  ackMessage = MyStreamAck,
  onFailureMessage = ex => MyStreamFail(ex)
) <-- "lifecycle" messages
...
//  numberSource.ask[Int](parallelism = 4)(simpleActor).to(Sink.ignore).run() // equivalent
  ...
  val actorPoweredSink = Sink.actorRefWithAck[Int](
    ...
    ackMessage = StreamAck, // interpreted as backpressure
