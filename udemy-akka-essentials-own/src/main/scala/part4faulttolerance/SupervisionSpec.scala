...
override val supervisionStrategy = OneForOneStrategy(maxNrOfRetries = 10, withinTimeRange = 1 minute) {
    case e: IllegalArgumentException => Restart
    // other cases here
}

OneForOneStrategy -> alternative: AllForOneStrategy , that extends the actions for all children
maxNrOfRetries, withinTImeRange -> retry this strategy

OneForOneStrategy is a partial function, is a decider = PartialFunction[Throwable, Directive]
...
object SupervisionSpec {
    ...
    class AllForOneSupervisor extends Supervisor {
        // applied for all the actors that failed
