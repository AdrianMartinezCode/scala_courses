...
Using context.stop
context.stop(child) <-- asynchronous - actor may continue to receive messages until actually stopped
context.stop(self) <-- will recursively stop children (asynchronously)

Using special messages
actor ! PoisonPill <-- handled as a normal message
actor ! Kill <-- instantly stops the actor, makes the actor throw an ActorKilledException

DeathWatch
context.watch(actor) <-- i'll receive a Terminated message when this actor dies
// can watch more than one actor, not necessarily children
// i'll receive it even if the actor is already dead
 */
