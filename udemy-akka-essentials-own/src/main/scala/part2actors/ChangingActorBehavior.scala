...
object ChangingActorBehavior extends App {
    ...
    class Counter extends Actor {
        ...
        override def receive: Receive = behavior(0)

        def behavior(count: Int): Receive = {
            case Increment => context.become(behavior(count + 1))
            case Decrement => context.become(behavior(count - 1))
            case Print => println(s"The current count are ${count}")
        }
    }
    ...
    class VoteAggregator extends Actor {
        override def receive: Receive = aggregate

        def aggregate: Receive = {
            case AggregateVotes(actors) => {
                context.become(recounting(actors, Map()))
                actors.foreach(_ ! VoteStatusRequest)
            }
        }

        def recounting(voters: Set[ActorRef], mapRecount: Map[String, Int]): Receive = {
            case VoteStatusReply(Some(candidate)) => {
                val votersNew = voters - sender()
                val votes = mapRecount + (candidate -> (mapRecount.getOrElse(candidate, 0) + 1))
                if (votersNew.isEmpty) {
                    context.become(finishedRecounted(votes))
                    this.context.self ! "Recount!"
                }
                else context.become(recounting(voters, votes))
            }
        }

        def finishedRecounted(votes: Map[String, Int]) : Receive = {
            case _ => println(votes)
        }
    }
